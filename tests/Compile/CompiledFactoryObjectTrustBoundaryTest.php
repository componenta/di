...
function compiledFactoryRuntimeBoundaryFingerprint(): string
{
...
it('reuses an already loaded generated class from an identical shard in another cache root', function (): void {
    ...
    $fingerprint = compiledFactoryRuntimeBoundaryFingerprint();
...
return %s::class;
PHP, $class, var_export($fingerprint, true), $class);
    $file = 'container.factories.' . substr(hash('sha256', $source), 0, 32) . '.php';
...
it('rejects an already loaded generated class when the second cache root contains different source', function (): void {
    ...
    $source = static function (string $value) use ($class, $fingerprint): string {
        return sprintf(<<<'PHP'
<?php
final class %s
{
...
return %s::class;
PHP, $class, var_export($fingerprint, true), var_export($value, true), $class);
    };
    $firstSource = $source('first');
    $secondSource = $source('second');
    $firstFile = 'container.factories.' . substr(hash('sha256', $firstSource), 0, 32) . '.php';
    $secondFile = 'container.factories.' . substr(hash('sha256', $secondSource), 0, 32) . '.php';
    file_put_contents($firstBase . '/' . $firstFile, $firstSource);
    file_put_contents($secondBase . '/' . $secondFile, $secondSource);
    $firstDefinition = (new CompiledFactoryDefinition($firstFile, $class, 'create'))->encode();
    $secondDefinition = (new CompiledFactoryDefinition($secondFile, $class, 'create'))->encode();

    try {
        $cache = [
            'version' => ContainerBuilder::CACHE_VERSION,
            ConfigKey::DEPENDENCIES => [ConfigKey::FACTORIES => ['entry' => $firstDefinition]],
        ];
        $secondCache = $cache;
        $secondCache[ConfigKey::DEPENDENCIES][ConfigKey::FACTORIES]['entry'] = $secondDefinition;
        $first = ContainerBuilder::configureFromCache(new Config([]), $cache, $firstBase)->build();
        ...
        @unlink($firstBase . '/' . $firstFile);
        @unlink($secondBase . '/' . $secondFile);
        @rmdir($firstBase);
        @rmdir($secondBase);
        @rmdir($root);
    }
